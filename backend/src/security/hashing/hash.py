...
class HashGenerator:
    def __init__(self):
        self._hash_ctx_layer_1: CryptContext = CryptContext(
            schemes=[getenv("HASHING_ALGORITHM_LAYER_1", "bcrypt")],  # default to bcrypt
            deprecated="auto"
        )
        self._hash_ctx_layer_2: CryptContext = CryptContext(
            schemes=[getenv("HASHING_ALGORITHM_LAYER_2", "argon2")],  # default to argon2
            deprecated="auto"
        )
        self._hash_ctx_salt: str = getenv("HASHING_SALT", "default-secret-salt")
